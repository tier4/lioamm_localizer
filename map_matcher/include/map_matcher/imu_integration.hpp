...
public:
    ...
    const Eigen::Matrix4d & odometry)
  {
    ...
    // IMU bias Factor
    auto bias_noise =
      (gtsam::Vector(6) << 1.2123362494392119e-04, 1.2123362494392119e-04, 1.2123362494392119e-04,
       8.6572985145653080e-05, 8.6572985145653080e-05, 8.6572985145653080e-05)
        .finished();
    gtsam::BetweenFactor<gtsam::imuBias::ConstantBias> imu_bias_factor(
      B(key_ - 1), B(key_), gtsam::imuBias::ConstantBias(),
      gtsam::noiseModel::Diagonal::Sigmas(sqrt(imu_integrated_ptr_->deltaTij()) * bias_noise));
    // add IMU Factor
    ...
    gtsam::Values initial_values;
    initial_values.insert(X(key_), odometry_pose);
  ...
  void reset() { optimizer_.reset(new gtsam::ISAM2(parameter_)); }
