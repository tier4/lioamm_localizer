#include "lio_iv/optimization.hpp"
...
Eigen::Matrix4f Optimization::update(
  const double & timestamp, const Eigen::Matrix4d & latest_frame, const gtsam::NavState state,
  const gtsam::imuBias::ConstantBias imu_bias)
{
  ...
  if (lidar_odom_buffer_.empty()) {
    ...
    gtsam::BetweenFactor<gtsam::Pose3> lidar_relative_factor(
      X(key_ - 1), X(key_), pose_from.between(pose_to),
      gtsam::noiseModel::Isotropic::Sigma(6, 1e3));
  ...
  return estimated_state.matrix().cast<float>();
}
